...
for expt = {'linASmall_2'}
    ...
    for i = 3:length(files1)
        ...
        for j = 3:length(files2)           
            ...
            files3 = dir(dir3);
                root = [dir3 '/' '222/big2a/0/split_333']; 
                    fprintf('\nDir: %s\n', root);
                    DIM = 64;
                    counter=0;
...
%                                 labels = permute(reshape(lab, [3 DIM DIM DIM]), [3,4,2,1]);
...
%                                 predictions = permute(reshape(pred, [DIM DIM DIM 3]), [3,2,1,4]);
                    ...
                    BrowseComponents('ii',rot90(fliplr(labelArr)),rot90(fliplr(predArr)));
        end
    end
end
